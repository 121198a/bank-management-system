...
const getEmployee = asyncHandler(async (req, res) => {
  ...
  return new ApiResponse(200, "Employee fetched successfully", {
    employee,
  }).send(res);
});
...
const updateEmployee = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      const employee = await EmployeeProfile.findById(req.params.id).session(
        session,
      );
      ...
      if (req.body.branch) {
        const branch = await Branch.findById(req.body.branch).session(session);
      ...
      if (req.body.permissions !== undefined) {
        employee.permissions = [...new Set(req.body.permissions)];
      }
    ...
    const result = await EmployeeProfile.findById(updatedEmployee._id)
      .populate(
        "user",
...
const deactivateEmployee = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return new ApiResponse(200, "Employee deactivated successfully").send(res);
  } finally {
    await session.endSession();
  }
});
...
const getMyEmployeeProfile = asyncHandler(async (req, res) => {
  const profile = await EmployeeProfile.findOne({ user: req.user._id })
    .populate("branch", "branchName branchCode ifsc city")
    .populate("department", "code name")
    .populate("manager", "employeeId");

  if (!profile)
    throw new ApiError(404, "No employee profile exists for this account");

  return new ApiResponse(200, "Employee profile fetched", {
    employeeId: profile.employeeId,
    ...
    allBranchAccess: profile.allBranchAccess,
    allDepartmentAccess: profile.allDepartmentAccess,
    status: profile.status,
  }).send(res);
});
